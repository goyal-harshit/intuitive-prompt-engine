...
_KEYS = ("openness", "expansion_rate", "vertical_velocity", "pointing_up", "pointing_forward",
         "circularity", "circle_overhead", "horizontal_travel", "depth_velocity",
         "tempo", "stillness", "separation")

# prototype: {feature: (target, weight)} — unlisted features are ignored
_PROTOTYPES: dict[MotionPrimitive, dict[str, tuple[float, float]]] = {
    MotionPrimitive.EXPAND: {"expansion_rate": (0.5, 3.0), "tempo": (0.4, 0.5), "separation": (1.2, 0.5)},
    ...
    MotionPrimitive.SWEEP_HORIZONTAL: {"horizontal_travel": (0.75, 3.0), "circularity": (0.1, 1.0),
                                       "tempo": (0.5, 0.5)},
    MotionPrimitive.PUSH_AWAY: {"depth_velocity": (-0.6, 3.0), "openness": (0.8, 1.0)},
    MotionPrimitive.PULL_IN: {"depth_velocity": (0.6, 3.0)},
    MotionPrimitive.FRAME_RECT: {"separation": (1.0, 1.5), "stillness": (0.7, 2.0),
                                 "openness": (0.35, 1.5)},
    MotionPrimitive.HOLD_STILL: {"stillness": (0.9, 3.0), "tempo": (0.03, 1.0)},
    MotionPrimitive.WAVE: {"tempo": (0.8, 2.0), "smoothness": (0.3, 1.0),
...
class PrototypeSegmenter(SequenceSegmenter):
    def __init__(self, min_score: float = 0.62, min_duration_s: float = 0.45,
                 hold_still_duration_s: float = 2.0) -> None:
    ...
    def update(self, fv: GestureFeatureVector) -> SequenceSegment | None:
        ...
        for prim, proto in _PROTOTYPES.items():
            s = _match(fv, proto)
            if s > best_score:
    ...
    def _flush(self, now: float) -> SequenceSegment | None:
        ...
        return SequenceSegment(
            id=f"seg_{uuid.uuid4().hex[:8]}", primitive=prim,
