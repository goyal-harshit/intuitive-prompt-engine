...
class TemplatePromptGenerator(PromptGenerator):
    ...
    async def generate(self, graph: SceneGraph) -> OptimizedPrompt:
        ...
        for obj in sorted(graph.objects.values(), key=lambda o: -o.salience):
            ...
            noun = {
                "celestial": "a luminous celestial body",
                "airborne": "a flying subject",
                "subject": "a striking central subject",
            }.get(obj.category, f"a {obj.category}")
        ...
        parts.append(
            style.value if style and style.confidence >= 0.3 else "cinematic digital painting"
        )
